...
void *game_action(void *arg)
{
    ...
    int socket_descriptor = client_id;
    ...
    while (1)
    {
        ...
        if (recv(socket_descriptor, &action, sizeof(int), 0) <= 0)
        {
            printf("Socket %d disconnesso. Partita %d terminata forzatamente.\n", socket_descriptor, current_game_id);
            quit_game(socket_descriptor, current_game_id);
        ...
        case CREATE:
            ...
            send(socket_descriptor, &current_game_id, sizeof(int), 0);
            break;
        case LIST:
            get_list_game(socket_descriptor);
            break;
        case JOIN:
        {
            int game_id;
            if (recv(socket_descriptor, &game_id, sizeof(int), 0) <= 0)
                break;
            current_game_id = game_id;
            join_game(client_id, game_id, socket_descriptor);
            break;
        }
        case MOVE:
        {
            ...
            current_game_id = game_id;
            move(game_id, socket_descriptor);
            break;
        }
        case REMATCH:
        {
            ...
            if (recv(socket_descriptor, &game_id, sizeof(int), 0) <= 0)
                break;
            do_rematch(game_id, socket_descriptor);
            break;
        }
        case APPROVE:
        {
            int game_id, response;
            recv(socket_descriptor, &game_id, sizeof(int), 0);
            recv(socket_descriptor, &response, sizeof(int), 0);
            if (response == 0)
            {
                current_game_id = game_id;
            }
            approve_join_request(game_id, socket_descriptor, response);
            break;
        }
        case CANCEL:
        {
            ...
            send(socket_descriptor, &msg, sizeof(int), 0);
            break;
        }
        default:
            break;
...
void join_game(int client_id, int game_id, int socket_descriptor)
{
    ...
    if (selected_game == NULL)
    {
        ...
        send(socket_descriptor, &err, sizeof(int), 0);
        return;
    }
    ...
    // Notifica P1
    int p1_socket_descriptor = selected_game->id_player1;
    int join_packet[2];
    join_packet[0] = MSG_JOIN_REQUEST;
    join_packet[1] = game_id;

    if ((send(p1_socket_descriptor, join_packet, sizeof(join_packet), MSG_NOSIGNAL) < 0))
    { // se la send a p1 non va buon fine, P1 si è disconesso
        ...
        send(socket_descriptor, &msg_error, sizeof(int), 0);
    ...
    {
        start_game(selected_game, socket_descriptor);
    } 
    else
    {
        int err = MSG_JOIN_DENIED;
...
void approve_join_request(int game_id, int socket_descriptor, int response)
{
    ...
    if (selected_game->state == ST_APPROVE)
    {
        if (response == 0)
        {
            ...
            start_game(selected_game, selected_game->id_player1);
        }
        else
        {
...
void send_board_to_socket(int socket_descriptor, Game *game, GameCommand cmd)
{
    ...
    send(socket_descriptor, board_update, sizeof(int) + strlen(board_update + sizeof(int)), 0);
}
...
void move(int game_id, int socket_descriptor)
{
    ...
    if (recv(socket_descriptor, &row, sizeof(int), 0) <= 0 || recv(socket_descriptor, &col, sizeof(int), 0) <= 0)
        return;
    ...
    if (!is_move_valid(selected_game, row, col, socket_descriptor))
    {
    ...
    selected_game->table[row][col] = (socket_descriptor == selected_game->id_player1) ? 'X' : 'O';
    selected_game->turn++;
...
int is_move_valid(Game *game, int row, int col, int player_socket_descriptor)
{
    ...
    int current_turn_socket_descriptor = (game->turn % 2 == 0) ? game->id_player1 : game->id_player2;
    if (player_socket_descriptor != current_turn_socket_descriptor)
    {
        return 0;
    }
...
int do_rematch(int game_id, int socket_descriptor)
{
    ...
    if (selected_game->id_player1 != socket_descriptor && selected_game->id_player2 != socket_descriptor)
    {
    ...
    {
        rematch_by_winner(selected_game, socket_descriptor, response);
        return 0;
    }
    else
    { // pareggio
        rematch_from_both(selected_game, socket_descriptor, response);
        return 0;
    }
}
...
int rematch_from_both(Game *game, int socket_descriptor, int response)
{
...
int game_over(Game *game, int winner_socket_descriptor)
{
    ...
    if (winner_socket_descriptor == game->id_player2)
    {
    ...
    else if (winner_socket_descriptor == game->id_player1)
    {
        game->id_player2 = -1;
    }
    ...
    return ask_rematch(game, winner_socket_descriptor);
}

int change_owner_game(Game *game)
{

    int tempsocket_descriptor = game->id_player1;
    game->id_player1 = game->id_player2;
    game->id_player2 = tempsocket_descriptor;
    return 0;
}

int ask_rematch(Game *game, int winner_socket_descriptor)
{
    ...
    if (winner_socket_descriptor == 0)
    {
        send(game->id_player1, &msg, sizeof(int), 0);
        send(game->id_player2, &msg, sizeof(int), 0);
    }
    else
    { // lo mando solo al proprietario (ossia il vincitore)
        send(game->id_player1, &msg, sizeof(int), 0);
    }
...
void start_game(Game *game, int player_socket_descriptor) {
        
        if(player_socket_descriptor == game->id_player1){
            int msg = MSG_START_PLAYER1;
            int cmd = CMD_PLAY;
            send(player_socket_descriptor, &msg, sizeof(int), 0);
            // Piccolo delay per assicurarsi che i pacchetti arrivino in ordine
            usleep(DELAY_SYNC_MS);
            send_board_to_socket(player_socket_descriptor, game, cmd);
        }

        if(player_socket_descriptor == game->id_player2){
